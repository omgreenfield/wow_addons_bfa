...
local function setupAIConfig(c)
	...
	Lib_UIDropDownMenu_Initialize(config.channeldropdown, function(self, level)   
		...
		for k,v in pairs(channelOptions) do
			...
			info.text = v
			info.value = v
			info.func = function(self) Lib_UIDropDownMenu_SetSelectedID(config.channeldropdown, self:GetID()) if self:GetID() < 8 then config.channelextrabox:Hide() else config.channelextrabox:Show() end end
