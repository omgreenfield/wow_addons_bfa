local name, SPELLDB = ...
SPELLDB.DRUID = {}
SPELLDB.DRUID.BALANCE = {}
SPELLDB.DRUID.FERAL = {}
SPELLDB.DRUID.GUARDIAN = {}
SPELLDB.DRUID.RESTORATION = {}

SPELLDB.DRUID.BALANCE.spells = {
	208683,
	22812 --[[Barkskin--]],        
	5487 --[[Bear Form--]],        
	768 --[[Cat Form--]],        
	1850 --[[Dash--]],        
	339 --[[Entangling Roots--]],        
	6795 --[[Growl--]],        
	194153 --[[Lunar Strike--]],        
	8921 --[[Moonfire--]],        
	24858 --[[Moonkin Form--]],        
	5215 --[[Prowl--]],        
	8936 --[[Regrowth--]],        
	2782 --[[Remove Corruption--]],        
	50769 --[[Revive--]],        
	190984 --[[Solar Wrath--]],        
	191034 --[[Starfall--]],        
	78674 --[[Starsurge--]],        
	93402 --[[Sunfire--]],        
	18960 --[[Teleport: Moonglade--]],        
	783 --[[Travel Form--]],        
	197524 --[[Astral Influence--]],        
	20484 --[[Rebirth--]],        
	194223 --[[Celestial Alignment--]],        
	29166 --[[Innervate--]],        
	231042 --[[Moonkin Form--]],        
	231049 --[[Starfall--]],        
	231021 --[[Starsurge--]],        
	77492 --[[Mastery: Starlight--]],        
	78675 --[[Solar Beam--]], 
};

SPELLDB.DRUID.FERAL.spells = {
	5487 --[[Bear Form--]],        
	106951 --[[Berserk--]],        
	768 --[[Cat Form--]],        
	1850 --[[Dash--]],        
	339 --[[Entangling Roots--]],        
	22568 --[[Ferocious Bite--]],        
	6795 --[[Growl--]],        
	8921 --[[Moonfire--]],        
	5215 --[[Prowl--]],        
	1822 --[[Rake--]],        
	8936 --[[Regrowth--]],        
	2782 --[[Remove Corruption--]],        
	50769 --[[Revive--]],        
	1079 --[[Rip--]],        
	5221 --[[Shred--]],        
	61336 --[[Survival Instincts--]],        
	106785 --[[Swipe--]],        
	18960 --[[Teleport: Moonglade--]],        
	106830 --[[Thrash--]],        
	5217 --[[Tiger's Fury--]],        
	783 --[[Travel Form--]],        
	131768 --[[Feline Swiftness--]],        
	16949 --[[Feral Instinct--]],        
	159286 --[[Primal Fury--]],        
	20484 --[[Rebirth--]],        
	231063 --[[Shred--]],        
	231283 --[[Swipe--]],        
	16864 --[[Omen of Clarity--]],        
	106898 --[[Stampeding Roar--]],        
	231055 --[[Tiger's Fury--]],        
	48484 --[[Infected Wounds--]],        
	231057 --[[Shred--]],        
	22570 --[[Maim--]],        
	231056 --[[Ferocious Bite--]],        
	106839 --[[Skull Bash--]],        
	77493 --[[Mastery: Razor Claws--]],        
	16974 --[[Predatory Swiftness--]],   
};

SPELLDB.DRUID.GUARDIAN.spells = {
	22812 --[[Barkskin--]],        
	5487 --[[Bear Form--]],        
	768 --[[Cat Form--]],        
	1850 --[[Dash--]],        
	339 --[[Entangling Roots--]],        
	22842 --[[Frenzied Regeneration--]],        
	6795 --[[Growl--]],        
	99 --[[Incapacitating Roar--]],        
	33917 --[[Mangle--]],        
	6807 --[[Maul--]],        
	8921 --[[Moonfire--]],        
	5215 --[[Prowl--]],        
	8936 --[[Regrowth--]],        
	2782 --[[Remove Corruption--]],        
	50769 --[[Revive--]],        
	61336 --[[Survival Instincts--]],        
	213771 --[[Swipe--]],        
	18960 --[[Teleport: Moonglade--]],        
	77758 --[[Thrash--]],        
	783 --[[Travel Form--]],        
	16931 --[[Thick Hide--]],        
	20484 --[[Rebirth--]],        
	192081 --[[Ironfur--]],        
	231064 --[[Mangle--]],        
	210706 --[[Gore--]],        
	106898 --[[Stampeding Roar--]],        
	231070 --[[Ironfur--]],        
	231065 --[[Lightning Reflexes--]],        
	106839 --[[Skull Bash--]],        
	155783 --[[Mastery: Nature's Guardian--]],        
	192083 --[[Mark of Ursol--]],   
};

SPELLDB.DRUID.RESTORATION.spells = {
	22812 --[[Barkskin--]],        
	5487 --[[Bear Form--]],        
	768 --[[Cat Form--]],        
	1850 --[[Dash--]],        
	339 --[[Entangling Roots--]],        
	6795 --[[Growl--]],        
	5185 --[[Healing Touch--]],        
	33763 --[[Lifebloom--]],        
	8921 --[[Moonfire--]],        
	88423 --[[Nature's Cure--]],        
	5215 --[[Prowl--]],        
	8936 --[[Regrowth--]],        
	774 --[[Rejuvenation--]],        
	50769 --[[Revive--]],        
	5176 --[[Solar Wrath--]],        
	93402 --[[Sunfire--]],        
	18562 --[[Swiftmend--]],        
	18960 --[[Teleport: Moonglade--]],        
	783 --[[Travel Form--]],        
	48438 --[[Wild Growth--]],        
	145108 --[[Ysera's Gift--]],        
	20484 --[[Rebirth--]],        
	231032 --[[Regrowth--]],        
	113043 --[[Omen of Clarity--]],        
	29166 --[[Innervate--]],        
	231040 --[[Rejuvenation--]],        
	102342 --[[Ironbark--]],        
	48500 --[[Living Seed--]],        
	102793 --[[Ursol's Vortex--]],        
	212040 --[[Revitalize--]],        
	145205 --[[Efflorescence--]],        
	77495 --[[Mastery: Harmony--]],        
	740 --[[Tranquility--]],      
};

SPELLDB.DRUID.BALANCE.talents = {
	205636 --[[Force of Nature--]],        
	202425 --[[Warrior of Elune--]],        
	202345 --[[Starlord--]],        
	108238 --[[Renewal--]],        
	102280 --[[Displacer Beast--]],        
	102401 --[[Wild Charge--]],        
	202157 --[[Feral Affinity--]],        
	197491 --[[Guardian Affinity--]],        
	197492 --[[Restoration Affinity--]],        
	5211 --[[Mighty Bash--]],        
	102359 --[[Mass Entanglement--]],        
	132469 --[[Typhoon--]],        
	114107 --[[Soul of the Forest--]],        
	102560 --[[Incarnation: Chosen of Elune--]],        
	202347 --[[Stellar Flare--]],        
	202342 --[[Shooting Stars--]],        
	202359 --[[Astral Communion--]],        
	202360 --[[Blessing of the Ancients--]],        
	202770 --[[Fury of Elune--]],        
	202354 --[[Stellar Drift--]],        
	202430 --[[Nature's Balance--]],   
};
SPELLDB.DRUID.FERAL.talents = {
	202021 --[[Predator--]],        
	202022 --[[Blood Scent--]],        
	155580 --[[Lunar Inspiration--]],        
	108238 --[[Renewal--]],        
	102280 --[[Displacer Beast--]],        
	102401 --[[Wild Charge--]],        
	197488 --[[Balance Affinity--]],        
	217615 --[[Guardian Affinity--]],        
	197492 --[[Restoration Affinity--]],        
	5211 --[[Mighty Bash--]],        
	102359 --[[Mass Entanglement--]],        
	132469 --[[Typhoon--]],        
	158476 --[[Soul of the Forest--]],        
	102543 --[[Incarnation: King of the Jungle--]],        
	52610 --[[Savage Roar--]],        
	202031 --[[Sabertooth--]],        
	202032 --[[Jagged Wounds--]],        
	202060 --[[Elune's Guidance--]],        
	236068 --[[Moment of Clarity--]],        
	155672 --[[Bloodtalons--]],        
	202028 --[[Brutal Slash--]],      
};
SPELLDB.DRUID.GUARDIAN.talents = {
	203953 --[[Brambles--]],        
	155835 --[[Bristling Fur--]],        
	203962 --[[Blood Frenzy--]],        
	204012 --[[Guttural Roars--]],        
	236748 --[[Intimidating Roar--]],        
	102401 --[[Wild Charge--]],        
	197488 --[[Balance Affinity--]],        
	202155 --[[Feral Affinity--]],        
	197492 --[[Restoration Affinity--]],        
	5211 --[[Mighty Bash--]],        
	102359 --[[Mass Entanglement--]],        
	132469 --[[Typhoon--]],        
	158477 --[[Soul of the Forest--]],        
	102558 --[[Incarnation: Guardian of Ursoc--]],        
	203964 --[[Galactic Guardian--]],        
	203974 --[[Earthwarden--]],        
	155578 --[[Guardian of Elune--]],        
	203965 --[[Survival of the Fittest--]],        
	204053 --[[Rend and Tear--]],        
	204066 --[[Lunar Beam--]],        
	80313 --[[Pulverize--]],       
};
SPELLDB.DRUID.RESTORATION.talents = {
	200383 --[[Prosperity--]],        
	102351 --[[Cenarion Ward--]],        
	207383 --[[Abundance--]],       
	108238 --[[Renewal--]],        
	102280 --[[Displacer Beast--]],        
	102401 --[[Wild Charge--]],        
	197632 --[[Balance Affinity--]],        
	197490 --[[Feral Affinity--]],        
	197491 --[[Guardian Affinity--]],        
	5211 --[[Mighty Bash--]],        
	102359 --[[Mass Entanglement--]],        
	132469 --[[Typhoon--]],        
	158478 --[[Soul of the Forest--]],        
	33891 --[[Incarnation: Tree of Life--]],        
	200390 --[[Cultivation--]],        
	207385 --[[Spring Blossoms--]],        
	197073 --[[Inner Peace--]],        
	155675 --[[Germination--]],        
	155577 --[[Moment of Clarity--]],        
	197061 --[[Stonebark--]],        
	197721 --[[Flourish--]],     
};

SPELLDB.DRUID.BALANCE.pvpTalents = {
	208683 --[[Gladiator's Medallion--]],   
	214027 --[[Adaptation--]],   
	196029 --[[Relentless--]],   

	213551 --[[Train of Thought--]],   
	213552 --[[Mind Quickness--]],   
	213553 --[[Initiation--]],   

	233752 --[[Ironfeather armor--]],  
	209730 --[[protector of the grove--]],   
	233754 --[[Celestial Guardian--]], 

	200567 --[[Crescent Burn--]],   
	200726 --[[Celestial Downpour--]], 
	233750 --[[Eclipse--]], 

	200549 --[[Prickling Thorns--]], 
	232546 --[[Dying stars--]],   
	209740 --[[Moonkin Aura--]],   

	209753 --[[Cyclone--]],   
	233755 --[[Deep roots--]],   
	209749 --[[Faerie Swarm--]],   
};

SPELLDB.DRUID.FERAL.pvpTalents = {
	208683 --[[Gladiator's Medallion--]],   
	214027 --[[Adaptation--]],   
	196029 --[[Relentless--]],   

	195416 --[[Hardiness--]],   
	195282 --[[Reinforced Armor--]],   
	195425 --[[Sparring--]],   

	236019 --[[Tooth and claw--]],   
	209730 --[[Protector of the Grove--]],   
	236696 --[[Thorns--]],   

	205673 --[[Savage Momentum--]],  
	213200 --[[Freedom of the Herd--]],   
	236012 --[[Malorne's Swiftness--]],   

	203052 --[[King of the Jungle--]],   
	236026 --[[Enraged maim--]],   
	236023 --[[Earthen Grasp--]],   

	203224 --[[Fresh Wound--]],   
	236020 --[[Ferocious Wound--]],   
	203242 --[[Rip and Tear--]],   
};

SPELLDB.DRUID.GUARDIAN.pvpTalents = {
	208683 --[[Gladiator's Medallion--]],   
	214027 --[[Adaptation--]],   
	196029 --[[Relentless--]],   

	195338 --[[Relentless Assault--]],   
	207017 --[[Alpha Challenge--]],   
	201259 --[[Toughness--]],   

	236148 --[[Roaring Speed--]],  
	236144 --[[Master Shapeshifter--]],   
	236012 --[[Malorne's Swiftness--]],    

	236180 --[[Den Mother--]],   
	202043 --[[Protector of the Pack--]],   
	201664 --[[Demoralizing Roar--]],   

	236153 --[[Raging Frenzy--]],   
	213951 --[[Clan Defender--]],   
	202110 --[[Sharpened Claws--]],  

	228431 --[[Charging Bash--]],   
	202226 --[[Entangling Claws--]],   
	202246 --[[Overrun--]],   
};
SPELLDB.DRUID.RESTORATION.pvpTalents = {
	208683 --[[Gladiator's Medallion--]],   
	214027 --[[Adaptation--]],   
	196029 --[[Relentless--]],  

	195330 --[[Defender of the Weak--]],   
	195483 --[[Vim and Vigor--]],   
	195606 --[[Inner Renewal--]],   

	233673 --[[Disentanglement--]],   
	203727 --[[Thorns--]],   
	209690 --[[Druid of the Claw--]],   

	33786 --[[Cyclone--]],   
	200931 --[[Encroaching Vines--]],   
	206852 --[[Deep Roots--]],  

	203454 --[[Fortified Bark--]], 
	203374 --[[Nourish--]],   
	203399 --[[Revitalize--]],   

	203553 --[[Focused Growth--]],   
	203624 --[[Early Spring--]],   
	203651 --[[Overgrowth--]],   
};

SPELLDB.DRUID.BALANCE.artifact = {202767, 202996,202940,214508,214514,202445,202918,203018,202384,202464,202466,202386,202890,202426,202302,213682,202433,};
SPELLDB.DRUID.FERAL.artifact = {210722, 210666,210676,210702,214736,210637,210638,210557,210631,210663,210575,210570,210579,210590,210650,210571,210593,};
SPELLDB.DRUID.GUARDIAN.artifact = {200851,200850,200854,200855,215061,200400,215799,200440,200402,200515,200409,200414,208762,214996,200415,200399,200395,};
SPELLDB.DRUID.RESTORATION.artifact = {208253,189870,189857,189849,222644,186320,189787,189760,189754,189744,186372,189768,186393,189749,189772,186396,189757,}


-- SPELLDB.DRUID.BALANCE.artifact = {202767}
-- SPELLDB.DRUID.BALANCE.majorTraits = {202996,202940,214508,}
-- SPELLDB.DRUID.BALANCE.traits = {214514,202445,202918,203018,202384,202464,202466,202386,202890,202426,202302,213682,202433,};

-- SPELLDB.DRUID.FERAL.artifact = {210722}
-- SPELLDB.DRUID.FERAL.majorTraits = {210666,210676,210702,}
-- SPELLDB.DRUID.FERAL.traits = {214736,210637,210638,210557,210631,210663,210575,210570,210579,210590,210650,210571,210593,};

-- SPELLDB.DRUID.GUARDIAN.artifact = {200851}
-- SPELLDB.DRUID.GUARDIAN.majorTraits = {200850,200854,200855,}
-- SPELLDB.DRUID.GUARDIAN.traits = {215061,200400,215799,200440,200402,200515,200409,200414,208762,214996,200415,200399,200395,};

-- SPELLDB.DRUID.RESTORATION.artifact = {208253}
-- SPELLDB.DRUID.RESTORATION.majorTraits = {189870,189857,189849,}
-- SPELLDB.DRUID.RESTORATION.traits = {222644,186320,189787,189760,189754,189744,186372,189768,186393,189749,189772,186396,189757,}






